...
- packet-capture is performed by an external packet-capture MCP server (not part of this stack) registered directly in the Open WebUI admin UI
...
Open WebUI -> external packet-capture MCP (http://172.16.0.70:8080/mcp)
```
...
The packet-capture MCP server is **not** part of this Compose stack. It is an
external MCP server (Streamable HTTP at `http://172.16.0.70:8080/mcp`, no auth)
registered directly in the Open WebUI admin UI under **Manage Tool Servers**.
See `../docs/incident-driven-pcap.md` for the registration steps and the
incident-driven capture playbook.
...
- The external packet-capture MCP (`http://172.16.0.70:8080/mcp`) is registered in the Open WebUI admin UI, not configured via `.env`.
...
### 8. External packet-capture MCP

The packet-capture MCP is **not** a Compose service. Confirm the host can reach
the external server and register it in the Open WebUI admin UI.
...
- the request reaches the server (an empty body to a plain GET is normal for an
  MCP Streamable HTTP endpoint that expects a session POST)

To register it in Open WebUI, go to **Admin Settings -> Manage Tool Servers**
and add an MCP server using the external URL:

```text
http://172.16.0.70:8080/mcp
```

See `../docs/incident-driven-pcap.md` for the full registration steps, the
system-prompt playbook, and the end-to-end incident-driven capture workflow.
...
### Packet-capture MCP tools fail

Check from the Open WebUI host:
...
- the host cannot route to `172.16.0.70:8080` (network/firewall between segments)
- the external packet-capture MCP server is down or restarting
- the MCP server was not added under **Manage Tool Servers**, or the URL/path is wrong
- the model was not given the capture playbook prompt, so it never calls the tool
