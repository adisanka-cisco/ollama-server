...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if not payload_tools:
        ...
        tools_dict = {}
        mcp_tools_dict = {}
        resolved_tool_ids = tool_ids or []

        if tool_ids:
            resolved_tool_ids = [
                tool_id for tool_id in tool_ids if not tool_id.startswith("server:mcp:")
            ]
            for tool_id in tool_ids:
                if tool_id.startswith("server:mcp:"):
                    server_id = tool_id[len("server:mcp:") :]
                    try:
                        resolved_mcp_tools = await resolve_mcp_server_tools(
                            request,
                            server_id,
                            user,
                            extra_params,
