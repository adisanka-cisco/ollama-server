...
def get_mcp_server_connection(
    ...
    for connection in request.app.state.config.TOOL_SERVER_CONNECTIONS:
        if connection.get("type", "") != "mcp":
            continue
        if not connection.get("config", {}).get("enable"):
            continue
...
def get_function_name_filter_list(connection: Dict[str, Any]) -> list[str]:
    function_name_filter_list = connection.get("config", {}).get(
        "function_name_filter_list", ""
    )

    if isinstance(function_name_filter_list, str):
        return [name for name in function_name_filter_list.split(",") if name]
    if isinstance(function_name_filter_list, list):
        return [name for name in function_name_filter_list if name]
...
async def resolve_mcp_server_tools(
    request: Request,
    server_id: str,
    user: UserModel,
    extra_params: dict,
    ...
    connection = get_mcp_server_connection(request, server_id)
    if connection is None:
        log.warning(f"MCP server data not found for {server_id}")
        return {}
    ...
    function_name_filter_list = get_function_name_filter_list(connection)
    mcp_tools_dict = {}

    for tool_spec in await list_mcp_tool_specs(
        request,
        server_id,
        user,
        extra_params,
        ...
        exposed_name = f"{server_id}_{function_name}"
        ...
        if tool:
            ...
            if tool_id.startswith("server:"):
                ...
                if type == "openapi":
                    ...
                    try:
                        mcp_tools_dict = await resolve_mcp_server_tools(
                            request,
                            server_id,
                            user,
                            extra_params,
                    ...
                    for function_name, tool_dict in mcp_tools_dict.items():
                        updated_function_name = function_name
                        while updated_function_name in tools_dict:
                            log.warning(
                                f"Tool {updated_function_name} already exists in another tools!"
                            )
                            updated_function_name = f"{server_id}_{updated_function_name}"
                            tool_dict["spec"]["name"] = updated_function_name

                        tools_dict[updated_function_name] = tool_dict
                else:
                    continue
